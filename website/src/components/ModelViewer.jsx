...
const ModelInner = ({
  ...
  useLayoutEffect(() => {
    ...
    const s = (1 / (sphere.radius * 2)) * scaleMultiplier;
    ...
    // After scaling, move turbine sub-assembly independently if offset requested
    if (turbineYOffset !== 0) {
      g.traverse(o => {
        if (o.name && (
          o.name.toLowerCase().includes('turbine') ||
          o.name.toLowerCase().includes('torus') ||
          o.name.toLowerCase().includes('sensor')
        )) {
          o.position.y += turbineYOffset;
        }
      });
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <ambientLight intensity={ambientIntensity} color="#00e5ff" />
        <directionalLight position={[10, 10, 10]} intensity={keyLightIntensity} castShadow color="#ffffff" />
        <directionalLight position={[-10, 10, -10]} intensity={fillLightIntensity} color="#0070fe" />
        <directionalLight position={[0, 4, -5]} intensity={rimLightIntensity} color="#ffffff" />
